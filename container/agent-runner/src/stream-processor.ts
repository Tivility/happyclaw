...
export class StreamEventProcessor {
   ...
   * 本地 sub_agent_result 提取路径，与上游 pendingSubAgentMessages 并存、互不覆盖。
   *
   ...
   *
   * 本地路径，与上游 processMainToolResults 并存：前者按 pendingAgentResults 命中
   * 提取 sub-agent 结果（sub_agent_result），后者补发主 Agent 的 tool_result trace。
   * 二者作用于不同 tool_use_id 集合，互不覆盖。
   */
  ...
  cleanup(): void {
    ...
    // 上游 pendingSubAgentMessages 路径：清定时器并清空。
