...
// upstream 把单一 output.md 拆成 assistant / proactive / task 三段（原符号
// OUTPUT_GUIDELINES 已不存在）。这里按同一口径选，默认走 assistant。
function buildGuidelinesBlock(
  runtime: 'claude' | 'codex' | 'grok',
  mode: 'assistant' | 'proactive' | 'task' = 'assistant',
): string {
  const output =
    mode === 'task'
...
function buildSecurityRulesPrompt(disableMemoryLayer = false): string {
  // 关掉记忆层时，安全提示里关于 memory/ 的约束不再适用，避免自相矛盾的指令。
  if (!disableMemoryLayer) return SECURITY_RULES;
  return SECURITY_RULES.split('\n')
...
class MessageStream {
  ...
  push(
    ...
    let effectiveText = prepareMessageStreamText({
      text,
      originalImageCount,
...
function createPreCompactHook(
  ...
  // 本地能力：记忆层开关（HAPPYCLAW_DISABLE_MEMORY_LAYER）与隐私模式。
  // 隐私模式下既不归档对话到 conversations/，也不触发 memory flush。
...
async function runQueryAttempt(
  ...
  // 本地能力开关：记忆层总闸 + 隐私模式（隐私模式不归档对话、不写 memory/）。
  ...
  // 本地：额外可访问目录由 runtime-memory 按 disableMemoryLayer 统一派生
  //（关掉记忆层时不再挂 memory/ 与 global/，避免 Agent 绕过开关读到旧记忆）。
  ...
  try {
    ...
    const sdkCompat = withHappyClawSubagentContract({
      ...
      systemPrompt,
      // 权限走本地 resolveClaudePermissionOptions（按工作区/隐私模式解析），
      // 不用 upstream 写死的 bypassPermissions —— 那会让隐私模式形同虚设。
    ...
    for await (const message of q) {
      ...
      if (suppressOutputAfterInterrupt && message.type !== 'system') {
        if (message.type === 'result') {
          ...
          resultReceivedAt = Date.now();
          // F1：新 result 到达 = 完成债已偿还，退出挂流态，
          // 回到正常收尾倒计时（否则后台任务不唤醒主 Agent 时流会挂满超时）。
...
async function main(): Promise<void> {
  // 本地能力开关：记忆层总闸（隐私模式在 containerInput 解析后才可得）。
  const disableMemoryLayer =
    process.env.HAPPYCLAW_DISABLE_MEMORY_LAYER === 'true';
