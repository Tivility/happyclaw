...
内容全部来自 2026-07 那次合并的实测：121 个 upstream 提交、341 个冲突块、147 个
本地提交，过程中抓出 **19 个真 bug**，其中 8 个是上线后用户反馈才发现的。每条原则
后面都附了当时的具体证据 —— 不是通用建议，是这个仓库踩过的坑。
