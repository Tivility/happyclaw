...
| B3.4 | 切换运行时（`/api/model`）后下一轮生效 | 实际 runtime 与绑定一致 | ✅ **已修**：切绑定时发 `_drain`，Claude 常驻进程中切 Codex，下一条立刻换（原先要等 30 分钟空闲超时） |
...
| # | 用例 | 期望 | 结果 |
|---|---|---|---|
| B4.1 | 创建定时任务（`once`，立即到期） | 创建成功 | ✅ 建成 |
| B4.2 | 任务被调度并执行 | 产生 `task_runs` 行 + 执行日志 | ✅ `status=delivered`，Agent 回「任务已执行」；触发消息带「不是用户新发来的指令」框定 |
| B4.3 | 任务执行期间持有租约 | `listHeldTaskLeases()` 非空 | ✅ 执行后租约已释放（`lease_owner=None`） |
...
| # | 用例 | 期望 | 结果 |
|---|---|---|---|
| B5.1 | 软删除任务 | 从活列表消失，出现在已删列表 | ✅ rev 1→2，活列表 0 条；无 revision 返 428、错 revision 返 409 |
| B5.2 | `/restore` 恢复 | 回到活列表 | ✅ rev 2→3，`deleted_at=None` |
| B5.3 | 保留期内不被回收 | `getPurgeableTasks` 不含它 | ✅ 单测覆盖 |
| B5.4 | 超保留期后回收 | 任务行 + flow 工作区一并清除 | ✅ 保留期 7 天 + `deleted_at` 推到 30 天前 → 重启回收，`task_runs` 子记录一并清除，日志有记录 |
...
| # | 用例 | 期望 | 结果 |
|---|---|---|---|
| B8.1 | Claude 三轮上下文延续 | 后轮记得前轮内容 | ✅ 记住了 → 4712 → 4812 |
| B8.2 | Codex 三轮上下文延续（单 turn re-spawn + native resume） | 同上 | ✅ 记住了 → 8305 → 8300 |
| B8.3 | Grok 三轮上下文延续 | 同上 | ✅ 记住了 → 6190 → 12380（ACP `session/load` 跨 re-spawn resume 成功） |
| B8.4 | 回复不重复 | 每轮一条 `agent_reply`、DB 一行 | ✅ **已修**：原先每轮广播两次、落两行 |
| B8.5 | 新建工作区 | 建成且可用 | ✅ `flow-ms381b96-x28m` |
| B8.6 | 工作区隔离 | 独立 native session / 消息历史 / 工作目录 | ✅ 三者都独立；跨区能读到数字是**全局记忆**的设计行为，非隔离破损 |
| B8.7 | `/clear` 清上下文 | 清后不记得 | ✅ 生效（复答出的内容来自全局记忆，非上下文残留） |
| B8.8 | 文件上传 / 下载 / 读 / 改 / 删 | 全链路可用 | ✅ 全通（路径为 base64url 段） |
| B8.9 | 系统路径写入防护（`CLAUDE.md`） | 拒绝 | ✅ 403 |
| B8.10 | 路径遍历防护 | 不泄露内容 | ⚠️ 拦住了但返回 500 而非 4xx —— 错误码语义不准，既有行为非回归 |
| B8.11 | 记忆检索 API | 能搜到 Agent 写入的内容 | ✅ 命中全局 `CLAUDE.md` |
| B8.12 | 凭据失效时的自愈 | 不误发、不丢消息 | ✅ 12 次认证失败全走指数退避静默重试，游标保持原位 |
| B8.13 | 凭据回写（CLI 未刷新时） | 不动配置 | ✅ access token 仍有效 → CLI 没刷新 → 回写正确地未触发 |
| B8.14 | Grok resume 失败 fallback | 自动退回新会话 | ⚠️ **已补代码**（原先只有 codex 有），但端到端未能触发 —— 凭据轮换会先让 session key 失配、直接走新会话（那是另一条正确路径）。判定条件由单测锁住 |
...
| 5 | 2026-07-27 | B8 多轮对话 + 全功能面 | **抓到 2 个 bug**：回复重复发送/落库 / 切模型要等 30 分钟才生效 |
...
- **Grok 凭据已恢复**：把终端 `~/.grok/auth.json` 经 `updateProviderSecrets` 灌进 provider 配置（加密存储），先在沙箱跑完 B8.3 验证可用，再迁到生产，全程只保留一条活链。CLI 期间未刷新（access token 仍有效），所以终端那份也还能用。
  - 准确的教训不是「不能复制凭据」——复制本身没问题，副本会接管那条链并自我维护；**危险的是刷新之后把副本丢掉**。这也正是本轮补的凭据回写要解决的（见 `docs/design-provider-credential-lifecycle.md`）。
