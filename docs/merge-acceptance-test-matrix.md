...
| B8.3 | Grok 三轮上下文延续 | 同上 | ❌ **未完成**：grok 凭据失效（见下方说明） |
...
- **Grok 凭据已失效，需重新 `grok login`**。原因是测试过程中把生产的 `auth.json` 复制到沙箱运行 —— x.ai 的 refresh token 是一次性轮换的，沙箱那次成功刷新消耗掉了它，之后再用旧快照就报 `Authentication required`。**教训：会轮换的凭据不能复制去测试。**
