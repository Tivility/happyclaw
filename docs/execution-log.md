...
第一次 review 只查了「代码对不对」，没查「改完之后用户能不能用上」。按后一个标准重查，发现五项建了但用户完全感知不到，修掉其中三项。

### 修 1 · 日志轮转实际不会按天触发（真 bug）

`setInterval(24 * 60 * 60 * 1000)` 从**进程启动**算起。服务当天重启了 13 次，每次重启计时器归零——所以它不是「每天一次」，是「连续运行满 24 小时才一次」，实际约等于永不触发。

**改法**：判断权交给脚本本身。
- `rotate-logs.sh --if-due`：当天已有归档则跳过
- 调度改为「启动后 30s 检查一次 + 每小时检查一次」

频繁问「到期了吗」而非一次性问「满 24 小时了吗」，使调度能扛住重启，同时天然杜绝同日重复轮转。

**实测**：重启后启动检查触发并正确跳过（当天已轮转）——正是原计时器永远等不到的那个时刻。

### 修 2 · turn_events 前端消费

D-3 建了表、写入、API，但**前端零消费**——轨迹依然看不到，等于白做。

新增 `TurnTracePanel`：AI 消息下方一个「执行轨迹」折叠条，**点开才请求**（一个会话可能几百轮，绝大多数不会被查看，全量预加载得不偿失）。

渲染取舍：
- 优先显示子 Agent 的结论（`subAgentResult.summary`）——这是最有价值的一行
- 隐藏 `tool_progress` / `task_progress` / `init`（回放时无独立意义）
- 空轨迹显示「这一轮没有记录到工具调用」，与「面板坏了」区分开——早于 turn_events 的消息本就没有轨迹
- 分享视图不显示（不该暴露工具输入）

### 修 3 · 投递失败可见

批次 8 记录了状态但没有任何界面暴露，「发送失败」依然只能靠对方说没收到才发现。

- `getMessagesForChat` 的两处查询补上 `delivery_status`
- `MessageBubble` 对 `failed` 显示红色「未送达」、对 `pending` 显示琥珀色「送达未确认」

`pending` 也显示的原因：一条始终没结算的发送，用户体感与失败完全相同。`sent` / `skipped` / `null` 不显示——尤其 `null` 意味着「未跟踪」而非「失败」。

**验收**：118 文件 / 1403 测试通过，构建通过，上线后启动轮转检查实测生效。
