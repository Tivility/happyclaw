...
// 本地删除了 WebDeps.sessions（upstream 的 folder→sessionId 单一映射只覆盖
// Claude）。多运行时的 native session 权威表是 conversation_runtime_sessions，
// 断言改为直查该表；deps.sessions 仅作为兼容占位保留。
...
describe('workspace interaction_mode API', () => {
  ...
  test('owner mode change quiesces around commit and invalidates sessions', async () => {
    ...
    sessions[folder] = 'sdk-session-before-switch';
    ...
    // 本地删了 WebDeps.sessions（upstream 的 folder→sessionId 单一映射只覆盖
    // Claude）。多运行时的权威表是 conversation_runtime_sessions，由
    // deleteWorkspaceSessions 一并清空 —— 这里直接查表验证。
    expect(
      db.getConversationRuntimeSessionCount(folder),
    ).toBe(0);
  });
