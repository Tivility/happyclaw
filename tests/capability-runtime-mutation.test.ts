...
describe('capability runtime mutation invalidation', () => {
  test('system MCP mutation targets every inherit or source-qualified custom Agent', async () => {
    ...
    // Claude）。会话失效统一走 deleteWorkspaceSessions，它一次清三张表，含
    // 多运行时的权威表 conversation_runtime_sessions —— 断言它对该工作区被调用。
    expect(state.deletedFolders).toContain('u1-inherit-folder');
