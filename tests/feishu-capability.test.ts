...
/**
 * 卡片形状归一化（来自 upstream 3fa8360）。
 *
 * 模型很容易一边选 schema 2.0、一边沿用旧版 interactive-card 的包装
 * （`action.actions[]` / `note` / `header.theme.color_style`）。飞书对这种混合
 * 形状直接回 HTTP 400，用户看到的现象是「研究做完了但卡片没发出来」。
 *
 * 归一化只做**等价改写**：能无损转换的才转，任何多余字段都原样保留，让飞书返回
 * 明确的校验错误，而不是 broker 悄悄吞掉内容。
 *
 * 走 runFeishuCapability 而不是直接测纯函数：函数存在但调用点没接上，是这次追
 * 版本反复出现的形态（关键行没跟过来）。从入口测才能同时锁住接线。
 */
describe('send_card 的旧版形状归一化', () => {
  const sentCard = (calls: Array<{ op: string; args: unknown }>) =>
    JSON.parse(
      (calls[0].args as { data: { content: string } }).data.content,
    ) as Record<string, never>;

  test('header.theme.color_style 改写为 template', async () => {
    const { calls } = await run('send_card', {
      card: {
        schema: '2.0',
        header: {
      ...
      card: {
        ...
        body: {
          elements: [
            ...
            {
              tag: 'note',
              elements: [{ tag: 'plain_text', content: 'Generated by HappyClaw' }],
            },
          ],
        },
      },
    });
    expect(sentCard(calls).body).toEqual({
      elements: [
        { tag: 'hr' },
        {
          tag: 'button',
  ...
  test('转换会丢内容时原样保留，交给飞书报明确错误', async () => {
    // theme 里多了 custom、action 上有 layout、note 里是图片而非纯文本 ——
    // 任何一种都无法无损改写，宁可原样发出去。
    const unsafeCard = {
      schema: '2.0',
      header: { theme: { color_style: 'green', custom: 'keep-me' } },
      body: {
        elements: [
          {
            tag: 'action',
            layout: 'bisected',
            actions: [
              { tag: 'button', text: { tag: 'plain_text', content: 'Keep layout' } },
            ],
          },
          {
            tag: 'note',
    ...
    const { calls } = await run('send_card', { card: unsafeCard });
    expect(sentCard(calls)).toEqual(unsafeCard);
  });

  test('非 schema 2.0 的卡片完全不碰', async () => {
    const legacy = { header: { template: 'blue' }, elements: [{ tag: 'hr' }] };
    const { calls } = await run('send_card', { card: legacy });
    expect(sentCard(calls)).toEqual(legacy);
  });
});
