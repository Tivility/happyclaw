...
describe('builtin Skill catalog bootstrap contract', () => {
  test('standard install and supported start paths materialize the pinned catalog', () => {
    ...
    // 锚点用正则而非 'start: ##' 字面量：本地给 start 加了三个 SDK 门禁前置依赖
    // （ensure-latest-sdk / ensure-latest-codex-sdk / check-container-sdk），
    // 目标行变成 `start: <deps> ## ...`，字面量匹配会落空并静默切出空串。
    const startAt = makefile.search(/^start:.*##/m);
