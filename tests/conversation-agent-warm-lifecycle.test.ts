...
// sendMessage() now consults the runtime-binding gate (a real SQLite query in
// src/db.ts). This test runs against the live GroupQueue without an
// initialized DB, so stub only that gate to "no pending binding" (false) —
// the常规 warm-reuse path under test. Everything else in db.js stays real.
vi.mock('../src/db.js', async (importOriginal) => {
  const actual = (await importOriginal()) as Record<string, unknown>;
  return {
    ...actual,
    hasPendingConversationRuntimeBinding: () => false,
  };
});
