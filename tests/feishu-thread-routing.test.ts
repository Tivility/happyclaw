...
describe('StreamingCardController Feishu thread reply', () => {
  ...
  // merge §7.2: trace link 为上游引入未启用特性。决策1 以本地外观为基线，终态卡用 metaRow 取代 footer，
  // 不渲染 trace footer（setTraceUrl/traceFooterLink 暂留为死代码）。若后续采纳 trace 特性，移除 .skip 并在 builder.ts 接 trace footer。
  test.skip('preserves trace link when usage patch updates a legacy completed card', async () => {
