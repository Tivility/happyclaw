...
describe('channel_mounts 回填覆盖无显式绑定的历史群', () => {
  test('binding_mode=single_context 且两个绑定字段都空时，按 folder 兜底建 mount', () => {
    db.initDatabase();

    // 复刻真实存量形态：靠 folder 直接路由的旧模型群，没有
    // target_agent_id / target_main_jid。工作区侧是 web:{folder}。
    db.setRegisteredGroup('web:legacy-home', {
      name: 'Legacy Home',
      folder: 'legacy-home',
    ...
    db.setRegisteredGroup('feishu:oc_legacy_no_binding', {
    ...
    // 不兜底的话这里是 undefined，新的路由解析会 fail-closed 拒收 ——
    // 该会话升级后完全收不到消息，且没有 ERROR 日志指向根因。
    const mount = db.getChannelMount('feishu:oc_legacy_no_binding');
    expect(mount).toBeTruthy();
    expect(mount?.workspace_jid).toBe('web:legacy-home');
    expect(mount?.routing_mode).toBe('single_session');
  });

  test('folder 找不到对应工作区时不建 mount（不能凭空指向不存在的目标）', () => {
    db.setRegisteredGroup('feishu:oc_orphan_folder', {
      name: 'Orphan',
    ...
    db.syncAllChannelMountsFromRegisteredGroups();
    expect(db.getChannelMount('feishu:oc_orphan_folder')).toBeFalsy();
  });
});
