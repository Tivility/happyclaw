...
async function fetchLiveChatInfo(
  ...
  const chatInfo = deps?.getChannelChatInfo
    ? unwrapChatProbe(await deps.getChannelChatInfo(imJid))
    ...
    const chatInfoPromises = candidates
      ...
      .map(async (g) => {
        ...
        const info = deps.getChannelChatInfo
          ? unwrapChatProbe(await deps.getChannelChatInfo(g.jid))
          : g.channel_type === 'feishu'
            ? await deps.getFeishuChatInfo!(user.id, chatId)
            : null;
        if (info) {
          // ChannelChatInfo 的字段可为 null，目标字段是 string | undefined
          g.avatar = info.avatar ?? undefined;
          g.chat_mode = info.chat_mode ?? undefined;
          g.group_message_type = info.group_message_type ?? undefined;
