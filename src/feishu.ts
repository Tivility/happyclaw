...
export function createFeishuConnection(
  ...
  async function sendTextToChat(chatId: string, text: string): Promise<void> {
    ...
    try {
      ...
      // 必须重新抛出：吞掉这个错误会让上层（斜杠命令的 durable inbox）以为回复
      // 送达成功，把消息标成 processed 而永不重试。区分 rejected（确定性拒绝，
      // 可安全重放回复）与 uncertain（投递结果未知，只能人工核对）。
      throw new FeishuTextDeliveryError(
  ...
  const connection: FeishuConnection = {
    ...
    async sendMessage(
      ...
      try {
        ...
        // 决策：本地原有 clearAckForTarget(chatId) 在此丢弃。upstream 的契约是
        // 投递失败不提交 cursor、该轮待重放，⏳ 应当留着；重试耗尽后由
        // onMaxRetriesExceeded → clearTrackedProcessingIndicators(chatJid)
        // 按会话整体清理（本地按 chat 清的严格超集）。
        throw err;
      }
    },
    ...
    async executeCapability(_context, request) {
      if (!client) throw new Error('Feishu client is not connected');
      // 阶段 5 待办：upstream 的 executeFeishuCapability 从 ChannelTurnContext
      // 取 chatId（多账号路由 + 防越权指定 chat）。本地实现仍从 request 取，
      // 单账号下等价；多账号落地时一并切换。
      return runFeishuCapability(client, request);
    },
