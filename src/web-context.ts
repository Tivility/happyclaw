...
export interface WebDeps {
  ...
  /**
   * 探测渠道会话元数据。返回 ChatProbe 而非裸对象：必须能区分
   * `gone`（404/被踢，确定性否定）与 `unknown`（token/网络/429/5xx，我方故障），
   * 否则瞬时故障会被当成群失效而误触自动解绑。
   */
  getChannelChatInfo?: (jid: string) => Promise<ChatProbe>;
