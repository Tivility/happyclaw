...
interface ChatState {
  ...
  enablePrivacy: (jid: string) => Promise<void>;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (jid: string, content: string, attachments?: Array<{ data: string; mimeType: string }>) => {
    try {
      ...
      type MessageCreateResponse =
        | ClearedResponse
        | { success: true; messageId: string; timestamp: string; handledCommand?: boolean }
        | { success: false };
  ...
  enablePrivacy: async (jid: string) => {
    try {
