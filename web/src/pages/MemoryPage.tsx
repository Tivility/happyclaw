      ...
      <div className="max-w-7xl mx-auto space-y-4">
        <Card>
          <CardContent>
            ...
            {isAdmin && (
              <div className="mt-3 rounded-lg border border-warning/20 bg-warning-bg px-3 py-2 text-xs leading-5 text-warning">
                管理员视图可能包含多个用户和工作区的记忆。编辑前请核对左侧路径与工作区名称；宿主机
                Claude 配置单独显示且在此页面只读。
              </div>
            )}
          </CardContent>
        </Card>
                        ...
                        {!extCollapsed && (
                          <div className="space-y-1 ml-1">
                            <div className="text-[11px] text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg px-2.5 py-1.5 mb-1">
                              ⚠️ host 模式下智能体可直接编辑这些 CLAUDE.md /
                              rules，改动会写回本机{' '}
                              <code className="font-mono">~/.claude/</code>
                              ，影响你这台机器上所有 Claude Code 项目。
                            </div>
                            {extResources.claudeMd && (
                              <button
                                onClick={() => {
