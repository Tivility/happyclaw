...
export const MessageBubble = memo(
  function MessageBubble({
    ...
    const agentIdentity = resolveAgentDisplayIdentity({
      // per-user AI 外观（CLAUDE.md §8.9）已在 auth store 里合进 appearance
      // （mergeUserAiAppearance），渲染层不再直读 currentUser.ai_*：身份解析
      // 收敛在 resolveAgentDisplayIdentity 一处，避免每个渲染点重复优先级规则。
      agentName,
        ...
        <div className="lg:flex lg:gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="overflow-hidden font-serif">
              ...
              {/* 执行轨迹 —— 这一轮实际做了什么。流式期间也能看到，但那份状态
                  随标签页消失；这里从 turn_events 回读。共享视图下隐藏（不得暴露
                  工具入参），无 turn_id 的旧消息跳过。 */}
